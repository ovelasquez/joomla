...
class com_SecuritycheckInstallerScript
{
	...
	public function preflight($type, $parent)
	{
		...
		if ( !version_compare(PHP_VERSION, '5.3.0', 'ge') ) {
			...
			return false;
		} else if ( (version_compare(JVERSION, '2.5.0', 'lt')) || (version_compare(JVERSION, '3.0.0', 'ge')) ) {
			// Only allow to install on Joomla! 2.5.0 or later, but not in 3.x branch
			Jerror::raiseWarning(null, "This version doesn't work in Joomla! 3.x branch");
			return false;
		}
	...
	function uninstall($parent){
		...
		$db = JFactory::getDbo();
		$columnName      = $db->nameQuote("extension_id");
		$tableExtensions = $db->nameQuote("#__extensions");
		$type 			 = $db->nameQuote("type");
		$columnElement   = $db->nameQuote("element");
		$columnType      = $db->nameQuote("folder");
