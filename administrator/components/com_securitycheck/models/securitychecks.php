...
*/
class SecuritychecksModelSecuritychecks extends JModel
{
/**
* Array de datos
* @var array
...
$database = $db->getEscaped($database);
$campo = $db->getEscaped($campo);
$nombre = $db->Quote($db->getEscaped($nombre));
...
/*
Comprueba si un string es un número válido. Esta función es útil para comprobar si la versión de nuestro componente alguna palabra como "Beta" o "RC". En
este caso, devolvemos "false" como resultado y la versión de nuestro componente será siempre vulnerable
 */
...
$database = $db->getEscaped($database);
$campo = $db->getEscaped($campo);
$nuevo_valor = $db->Quote($db->getEscaped($nuevo_valor));
$campo_set = $db->getEscaped($campo_set);
...
// Sanitizamos las entradas
$nombre = $db->getEscaped($nombre);
$version = $db->getEscaped($version);
$tipo = $db->getEscaped($tipo);
